...
def player(prev_play, opponent_history=[], opponent="quincy"):
    ...
    if len(opponent_history) > 1:
        ...
        # Place more emphasis on learning from losses
        if reward == -1:
            alpha_loss = 0.2  # higher learning rate for losses
            old_value = q_table[last_state][guess]
            future_value = max(q_table[state].values())
            q_table[last_state][guess] = old_value + alpha_loss * (reward + gamma * future_value - old_value)
        else:
            old_value = q_table[last_state][guess]
            future_value = max(q_table[state].values())
